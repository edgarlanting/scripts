...
write_licenses() {
    ...
    for pkg in $(image_packages "$1" | sort); do
        ...
        # Filter out directories, cut type marker, quote "\", and convert line breaks to "\n"
        local files="$(get_metadata "$1" "${pkg}" CONTENTS | grep -v '^dir ' | cut -d ' ' -f 2- | sed 's#\\#\\\\#g' | tr '\n' '*' | sed 's/*/\\n/g')"
        ...
        echo -n "], \"description\": \"${description}\", \"homepage\": \"${homepage}\", \"source\": \"${src_uri}\", \"files\": \"${files}\"}"
    done >> "$2"
...
# The generation of the metadata in write_licenses takes a while because the invocation of the portage wrapper
# is slow etc., thus, just reuse the JSON data which is also convenient with "jq" string substitution.
insert_licenses() {
    local json_input="$1"
    local root_fs_dir="$2"
    local output="${root_fs_dir}"/usr/share/licenses/INFO
    sudo mkdir -p "${root_fs_dir}"/usr/share/licenses/common
    echo "Flatcar Container Linux distributes software from various projects." | sudo_clobber "${output}"
    echo "The following list of projects shows their licenses, how to obtain the source code," | sudo_append "${output}"
    echo "and which binary files in Flatcar Container Linux were created from it." | sudo_append "${output}"
    echo "The license texts are available under /usr/share/licenses/common/ (read with \"less NAME.gz\")." | sudo_append "${output}"
    echo "Build system files and patches used to build these projects are located at:" | sudo_append "${output}"
    echo "https://github.com/flatcar-linux/coreos-overlay/" | sudo_append "${output}"
    echo "https://github.com/flatcar-linux/portage-stable/" | sudo_append "${output}"
    echo "https://github.com/flatcar-linux/scripts/" | sudo_append "${output}"
    echo "Information on how to build Flatcar Container Linux can be found under:" | sudo_append "${output}"
    echo "https://docs.flatcar-linux.org/os/sdk-modifying-flatcar/" | sudo_append "${output}"
    echo | sudo_append "${output}"
    jq -r '.[] | "\(.project):\nDescription: \(.description)\nLicenses: \(.licenses)\nHomepage: \(.homepage)\nSource code: \(.source)\nFiles:\n\(.files)\n"' \
        "${json_input}" | sudo_append "${output}"
    # Compress the file from 2.1 MB to 0.39 MB; reading with "less INFO.bz2" works on Flatcar
    sudo lbzip2 -9 "${output}"
